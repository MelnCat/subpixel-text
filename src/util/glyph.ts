...
export class GlyphRow {
	public readonly width = Math.ceil(this.glyphs.reduce((l, c) => l + c.width, 0) + this.gap * (this.glyphs.length - 1)) / 3
	readonly height = this.glyphs[0]?.data.length ?? 0;
	constructor(public readonly glyphs: Glyph[], public readonly gap: number) {
	...
	draw(setPixel: (x: number, y: number, color: number) => void) {
		...
		const mergedRows = this.glyphs.map(x => x.data).reduce((l, c) => l.map((x, i) => x.concat(Array(this.gap).fill(false)).concat(c[i])));
